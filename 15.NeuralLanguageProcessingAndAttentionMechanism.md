...
Since RNNs are no longer widely used nowadays, I will introduce the Attention model and the Transformer model directly in the following two chapters. Don't worry! I believe it does not matter even if you have not learned anything about RNN yet.

> **The Homework from Andrew's RNN Course**
> 
> I completed a homework project from the RNN course, where I built a date conversion model using RNN and an attention mechanism. I created a Jupyter notebook with step-by-step explanation, which you can read [here](https://github.com/ATreep/DateRecogAttentionModel/blob/main/build_network_step_by_step.ipynb).
...
4. See the amounts of attentions as weights and compute a context amount $c^{<t>}=\sum_{t'=1}^{T_{x}}\alpha^{<t,t'>}a^{<t'>}$, which is the output of an attention block of a translation model
...
<img src="assets/image-1756456500957.png"  />
Each block in encoder RNN produces **an amount of attention $\alpha^{<t,t'>}$**. $\alpha^{<t,t'>}$ is what the amount of attention $y^{<t>}$ (decoder's output) should pay to $a^{<t'>}$ (state of Bi-RNN block $t'$, which is a combination of $(\overrightarrow{a}^{<t'>},\overleftarrow{a}^{<t'>})$, where each of the amount $a$ is one direction's RNN state).
<img src="assets/image-1756457034011.png"  />
...
Known that $\alpha^{<t,t'>}$ is a weight, so the sum of them should be 1. We calculate the softmax of $e^{<t,t'>}$ to have the normalized $\alpha^{<t,t'>}$.
