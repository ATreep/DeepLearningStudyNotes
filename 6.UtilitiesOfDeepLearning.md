...
-  $\parallel w^{[l]}\parallel ^{2}_F$ is Frobenius Norm (using $\parallel w\parallel ^{2}_2$, i.e. Eclidean Norm or L2 Norm, if $w$ is a vector, such as when logistic regression), which equals to $\sum_{i=1}^{n^{[l]}}\sum_{j=1}^{n^{[l-1]}}(w_{ij}^{[l]})^2$. In Python, we can use `np.sum(np.square(parameters[f"W{l}"]))` to calculate it.
